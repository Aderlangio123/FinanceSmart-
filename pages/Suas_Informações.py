...
def exibir_informacoes():
    if "email" not in st.session_state or "nome_de_usuario" not in st.session_state or "senha" not in st.session_state:
        ...
        # Exibir a foto do cliente se disponível
        if foto is not None:
            st.image(foto, caption="Foto de Perfil", width=300 )
        else:
            st.warning("Nenhuma foto de perfil foi carregada. Envie sua foto no cadastro.")
